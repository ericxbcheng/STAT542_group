...
knitr::opts_chunk$set(fig.height = 6, fig.width = 8, out.width = '50%',fig.show = "hold", fig.align = "center")
options(width = 90)
```
...
# Project Description and Summary
  ...
  With the goal of enhancing interpretability of the models, skin lesion was automatically identified by border detection and 22 features were engineered to reflect skin lesion’s shape characteristics (perimeter, area, border irregularity, shape compactness) and color characteristics (pixel mean and standard deviation of normalized RGB, HSV, CIE L*a*b color channels). [Model training part]
...
for(i in 1:length(files_malignant)){
  load.image(file = files_malignant[i]) %>%
      ...
      dilate_square(im = ., size = 3) %>%
      medianblur(im = ., n = 5) %>%
      save.image(im = ., file = paste0("D:/STAT542/my_malignant/","malignant", i, ".png"))
}
```
...
```{r, echo = FALSE, eval = TRUE, fig.keep = "hold", out.width = "50%"}
...
```{r, echo = FALSE, eval = TRUE}
mod_svm = readRDS(file = "D:/STAT542/mod_svmRadial_roc_cv_10.rds")
fit.glm = readRDS(file = "D:/STAT542/Q1_glm.rds")
```
...
plot(fit.glm$glmnet.fit, "lambda", main = "Fig.4 Plot of lambda versus coefficients.")
abline(v = log10(fit.glm$lambda.min), col = "red")

# Table 1
kable_styling(kable_input = kable(num_df, format = "latex", caption = ""), full_width = TRUE, latex_options = "hold_position")
...
\begin{tabular}{lllll}
\hline
\textbf{F1 Score}                     & \textbf{Accuracy} & \textbf{Sensetivity} & \textbf{Precision} & \textbf{F1 Score} \\ \hline
\hline
\textbf{32 Dimension Dense Embedding} & 0.7               & 0.7                  & 0.7                & 0.7               \\ \hline
\end{tabular}
\end{table}


```{r, eval = TRUE}
f_param = function(mod){
  return(str_c(names(mod$bestTune), mod$bestTune, sep = " = ", collapse = ", "))
}

Q1_table = tibble(Model = c("Logistic Regression with Lasso", "SVM with radial basis kernel"), 
                  Parameters = c(paste0("lambda = ", fit.glm$lambda.min), f_param(mod_svm)),
                  Test_Accuracy = c(result_lasso$overall["Accuracy"], result_svm$overall["Accuracy"]),
                  Sensitivity = c(result_lasso$byClass["Sensitivity"], result_svm$byClass["Sensitivity"]),
                  Specificity = c(result_lasso$byClass["Specificity"], result_svm$byClass["Specificity"]))
# Table 2
kable_styling(kable_input = kable(x = Q1_table, format = "latex", caption = "Summary of models"), full_width = TRUE)
```
...
show_border(im = im_demo, bool = ROI) %>% plot(main = "Fig.3 Demonstration of border detection.")
```
...
```
  ...
  A single tree model was trained with the complexity parameter as the tuning parameter. 5-fold cross-validation was used to tune over such complexity parameter values as 0, 0.001, 0.005, 0.01, and 0.05. The cross validation result shows that the ROC was maximized when `cp` = `r mod_cart$bestTune` and thus this was chosen for the final tree model (**Fig.7**). The decision tree 

```{r, eval = TRUE}
plot(mod_cart, main = "Fig.7 Cross-validation of the tree model.")
rpart.plot(x = mod_cart$finalModel, main = "Fig.8 The decision tree.")
```
...
plot(feature.perf, colorize=TRUE)
...
tst_data_pca = prcomp(x = tst_data, center = TRUE, scale. = TRUE)$x
pred_ada = predict(object = mod_ada$finalModel, newdata = as.data.frame(tst_data_pca))
result_ada = confusionMatrix(data = as.factor(pred_ada$class), reference = as.factor(tst_y), positive = "m")
...
pred_plr2 = predict(object = mod_plr2$finalModel, newdata = tst_data2)
