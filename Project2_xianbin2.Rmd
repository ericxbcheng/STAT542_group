...
```{r}
rm_border = function(im, thresh_black, thresh_perc){
  ...
  rm_pix_x = rowMeans(bool_black) > thresh_perc
  rm_pix_y = colMeans(bool_black) > thresh_perc
  return(crop.borders(im = im, nx = sum(rm_pix_x) / 2, ny = sum(rm_pix_y) / 2))
}

test2 = rm_border(im = test, thresh_black = 0.1, thresh_perc = 0.12) %>% plot()
```
...
for(i in 1:length(files_malignant)){
  ...
  my_crop(im = load.image(files_malignant[i]), length = 600, width = 450) %>% imager::save.image(im = ., file = paste0("D:/STAT542/my_malignant/malignant", i, ".jpeg"))
